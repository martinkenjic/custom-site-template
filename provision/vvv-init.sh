...
setup_nginx_folders() {
  ...
  echo " * Creating wordpress folder if it doesn't exist already"
  noroot mkdir -p "${VVV_PATH_TO_SITE}/wordpress"
}
...
install_wp() {
  ...
  echo " * Installing using wp core install --url=\"${DOMAIN}\" --title=\"${SITE_TITLE}\" --admin_name=\"${ADMIN_USER}\" --admin_email=\"${ADMIN_EMAIL}\" --admin_password=\"${ADMIN_PASSWORD}\" --path=\"${VVV_PATH_TO_SITE}/wordpress\""
  ...
  if [ "${WP_TYPE}" = "subdomain" ]; then
    echo " * Running Multisite install using wp core multisite-install --subdomains --url=\"${DOMAIN}\" --title=\"${SITE_TITLE}\" --admin_name=\"${ADMIN_USER}\" --admin_email=\"${ADMIN_EMAIL}\" --admin_password=\"${ADMIN_PASSWORD}\" --path=\"${VVV_PATH_TO_SITE}/wordpress\""
    ...
    echo " * Running Multisite install using wp core ${INSTALL_COMMAND} --url=\"${DOMAIN}\" --title=\"${SITE_TITLE}\" --admin_name=\"${ADMIN_USER}\" --admin_email=\"${ADMIN_EMAIL}\" --admin_password=\"${ADMIN_PASSWORD}\" --path=\"${VVV_PATH_TO_SITE}/wordpress\""
...
if [ "${WP_TYPE}" == "none" ]; then
  ...
  if [[ ! -f "${VVV_PATH_TO_SITE}/wordpress/wp-load.php" ]]; then
    download_wordpress "${VVV_PATH_TO_SITE}/wordpress" "${WP_VERSION}" "${WP_LOCALE}"
  fi
  ...
  if ! $(noroot wp core is-installed ); then
    ...
    if [ -f "${VVV_PATH_TO_SITE}/wordpress/wp-content/database.sql" ]; then
      restore_db_backup "${VVV_PATH_TO_SITE}/wordpress/wp-content/database.sql"
